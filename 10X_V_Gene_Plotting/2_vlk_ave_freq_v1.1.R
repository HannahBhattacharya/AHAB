...
vlk.id.list <- c("IGKV1-5", "IGKV1-6", "IGKV1-8", "IGKV1D-8", 
...
sum.vlk.freq.make.csv <- function(file, the.dir) {
  ...
  sum.file.vlk.id.df <- df %>%
    group_by(single_call) %>%
    summarise(
      observations = n(),
      vlk.id.freq.sum = sum(proportion)
    )
  sum.file.vlk.id.df
  
  #this generates filename for csv files
  filename.final <- basename(file)
  vlk.fam.freq.sum.filename <- paste(dir.out,filename.final,
                                     "_vlk_fam_freq_sum",".csv",sep="")
  vlk.id.freq.sum.filename <- paste(dir.out,filename.final,
                                    "_vlk_id_freq_sum",".csv",sep="")

  #this adds sample ID column to both summary files
  file.id <- c(rep_len(filename.final, 
                       length(as.character(sum.file.vlk.id.df$single_call))))
  file.fam <- c(rep_len(filename.final, 
                        length(as.character(sum.file.vlk.fam.df$vlk_family))))
  sum.file.vlk.fam.df <- data.frame(file.fam, sum.file.vlk.fam.df)
  sum.file.vlk.id.df <- data.frame(file.id, sum.file.vlk.id.df)
  ...
  colnames(full.df) [1:4] <- c("file.id", "vlk_family", "count", "frequency")
  
  #turning NA to 0 so averages calculate properly
  vlk.fam.freq.df <- full.df %>%
    complete(file.id, nesting(vlk_family),
             fill = list(count = 0,
                         frequency = 0))
  ...
  #so that frequencies calculate properly when
  #averaged across donors
  vlk.id.list.df <- data.frame(vlk.id.list)
  increase.id.len <- c(rep_len(filename.final, 
                               length(as.character(vlk.id.list.df$vlk.id.list))))
  add.id.full.vlk.id.df <- data.frame(vlk.id.list.df, increase.id.len)
  full.df <- merge(add.id.full.vlk.id.df, sum.file.vlk.id.df,
                   by.x = "vlk.id.list",
                   by.y = "single_call",
                   all.x = TRUE)
  drops <- c("file.id")
  ...
  colnames(full.df) [1:4] <- c("file.id", "vlk_gene", "count", "frequency")
  
  #turning NA to 0 so averages calculate properly
  vlk.id.freq.df <- full.df %>%
    complete(file.id, nesting(vlk_gene),
             fill = list(count = 0,
                         frequency = 0))
  
  #write csv files out
  write.csv(vlk.fam.freq.df, file = vlk.fam.freq.sum.filename)
  write.csv(vlk.id.freq.df, file = vlk.id.freq.sum.filename)
}
...
vlk.fam.freq.sum.filename <- paste(dir.out, filename.final,
                                   "vlk_fam_freq_sum",".csv",sep="")
vlk.id.freq.sum.filename <- paste(dir.out, filename.final,
                                  "vkl_id_freq_sum",".csv",sep="")
