...
#lambda
file.fam <- paste0(dir.in, "/filtered_combined_vl_fam_freq.csv")
file.gene <- paste0(dir.in, "/filtered_combined_vl_gene_freq.csv")
...
ggsave("vl_family_ave.pdf", plot = last_plot(), path = dir.out)
...
ggplot(data, aesthetic) + geometry + 
  theme(axis.text.x=element_text(angle=90, hjust=1))
ggsave("vl_gene_ave.pdf", plot = last_plot(), path = dir.out)

#to plot with error bars
#ggplot(df, aesthetic) + geometry + geometry.error + theme(axis.text.x=element_text(angle=90, hjust=1))

#--------------------------------------
#to plot geom boxplot based on cd21low.T1D > cd21low.FDR for Vk and Vl separately
file.enriched.subset <- paste0(dir.in, "/vl_gene_plot_when_cd21low_T1D_increased.csv")
data.enriched.subset <- data.frame(read.csv(file.enriched.subset, 
                                            header = TRUE))
...
ggsave("vl_gene_plot_when_cd21low_T1D_increased.pdf", 
