...
vh.id.list <- c("IGHV1-18", "IGHV1-2", "IGHV1-24", "IGHV1-3", 
...
sum.vh.freq.make.csv <- function(file, the.dir) {
  ...
  vh.fam.freq.sum.filename <- paste(dir.out,filename.final,
                                    "_vh_fam_freq_sum",".csv",sep="")
  vh.id.freq.sum.filename <- paste(dir.out,filename.final,
                                   "_vh_id_freq_sum",".csv",sep="")

  #this adds sample ID column to both summary files
  file.id <- c(rep_len(filename.final, 
                       length(as.character(sum.file.vh.id.df$single_call))))
  file.fam <- c(rep_len(filename.final, 
                        length(as.character(sum.file.vh.fam.df$vh_family))))
  sum.file.vh.fam.df <- data.frame(file.fam, sum.file.vh.fam.df)
  sum.file.vh.id.df <- data.frame(file.id, sum.file.vh.id.df)
  ...
  colnames(full.df) [1:4] <- c("file.id", "vh_family", "count", "frequency")
  
  #turning NA to 0 so averages calculate properly
  vh.fam.freq.df <- full.df %>%
    complete(file.id, nesting(vh_family),
             fill = list(count = 0,
                         frequency = 0))
  ...
  #so that frequencies calculate properly when
  #averaged across donors
  vh.id.list.df <- data.frame(vh.id.list)
  increase.id.len <- c(rep_len(filename.final, 
                               length(as.character
                                      (vh.id.list.df$vh.id.list))))
  add.id.full.vh.id.df <- data.frame(vh.id.list.df, increase.id.len)
  full.df <- merge(add.id.full.vh.id.df, sum.file.vh.id.df,
                   by.x = "vh.id.list",
                   by.y = "single_call",
                   all.x = TRUE)
  drops <- c("file.id")
  ...
  colnames(full.df) [1:4] <- c("file.id", "vh_gene", "count", "frequency")
  
  #turning NA to 0 so averages calculate properly
  vh.id.freq.df <- full.df %>%
    complete(file.id, nesting(vh_gene),
             fill = list(count = 0,
                         frequency = 0))
  
  #write csv files out
  write.csv(vh.fam.freq.df, file = vh.fam.freq.sum.filename)
  write.csv(vh.id.freq.df, file = vh.id.freq.sum.filename)
}
...
file <- "~/desktop/10X_BCR_pipelines/output/vh/4025-RB-1_all_1_vh_single_call_freq.csv"
df <- read.csv(file)
vh.fam.list <- c("IGHV1", "IGHV2", "IGHV3", "IGHV3/OR16", 
                 "IGHV4", "IGHV5", "IGHV6", "IGHV7")
#need to check the list below for completeness!
...
vh.fam.freq.sum.filename <- paste(dir.out,filename.final,"vh_fam_freq_sum",".csv",sep="")
vh.id.freq.sum.filename <- paste(dir.out,filename.final,"vh_id_freq_sum",".csv",sep="")

#this adds sample ID column to both summary files
file.id <- c(rep_len(filename.final, length(as.character(sum.file.vh.id.df$single_call))))
file.fam <- c(rep_len(filename.final, length(as.character(sum.file.vh.fam.df$vh_family))))
sum.file.vh.fam.df <- data.frame(file.fam, sum.file.vh.fam.df)
sum.file.vh.id.df <- data.frame(file.id, sum.file.vh.id.df)
...
#averaged across donors
vh.fam.list.df <- data.frame(vh.fam.list)
increase.id.len <- c(rep_len(filename.final, length(as.character(vh.fam.list.df$vh.fam.list))))
add.id.full.vh.fam.df <- data.frame(vh.fam.list.df, increase.id.len)
full.df <- merge(add.id.full.vh.fam.df, sum.file.vh.fam.df,
                 by.x = "vh.fam.list",
                 by.y = "vh_family",
...
#averaged across donors
vh.id.list.df <- data.frame(vh.id.list)
increase.id.len <- c(rep_len(filename.final, length(as.character(vh.id.list.df$vh.id.list))))
add.id.full.vh.id.df <- data.frame(vh.id.list.df, increase.id.len)
full.df <- merge(add.id.full.vh.id.df, sum.file.vh.id.df,
                 by.x = "vh.id.list",
                 by.y = "single_call",
