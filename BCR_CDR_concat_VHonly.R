...
#remove.delimiters <- function(data){
#  data <- str_remove_all(data, "[(]")
#  data <- str_remove_all(data, "[)]")
#}
#new.df <- data.frame(df[1], lapply(df[2:7], remove.delimiters) )
...
#concatenate all three VH CDRs together into one column
...
filename<- paste(dir.out,"VH.CDR123.concat",".fasta",sep="")
...
#CDR to help with alignment. This requires both ends
# of CDR to be defined with ( )
...
filename<- paste(dir.out,"VH_CDR123_concat_add_anchor",".fasta",sep="")
#write fasta file
write.fasta(data, filename)
